...
<div class="top-bar py-4 flex justify-between items-center">
  ...
  <span class="flex items-center gap-4">
    <button on:click={callDelete} class="flex items-center gap-1 p-2">
      <Trash2 size={18} /> Delete</button
    >
    <Button variant={"brand"} class="flex items-center gap-1 p-2 rounded-md">
...
<Table.Root class="mx-auto w-full flex flex-col pb-10">
  <Table.Header
    class="border-2 border-[#e4e4e4] border-solid rounded-lg dark:border-none dark:bg-[#e4e4e4]/[.25]"
  >
    <Table.Row
      class="bg-transparent flex items-center justify-between p-3 w-full"
    >
      <Table.Head
        class="text-[#727272] h-full w-[5%] text-start dark:text-primary"
        ><input
          type="checkbox"
      ...
      <Table.Head
        class="text-[#727272] dark:text-[#FFFFFF]/70 text-start h-full w-[9%]"
        >Username</Table.Head
      >
      <Table.Head
        class="text-[#727272]  dark:text-[#FFFFFF]/70 text-center h-full w-[12.5%]"
        >Email</Table.Head
      >
      <Table.Head
        class="text-[#727272]  dark:text-[#FFFFFF]/70 text-center h-full w-[12.5%]"
      ...
      >
      <Table.Head
        class="text-[#727272]  dark:text-[#FFFFFF]/70 text-center pr-0 h-full w-[12.5%]"
    ...
    >
      {#each filteredData as user}
        ...
        >
          ...
          <Table.Cell
            class="text-black h-full w-[12.5%] text-start dark:text-white pl-7"
            ><span class="flex items-center capitalize font-semibold">
              {user.username === "" ? "N/A" : user.username}
            </span>
          </Table.Cell>
          <Table.Cell class="text-[#727272] h-full text-sm w-[10%] text-start">
            <Popover.Root>
              <Popover.Trigger
                ...
                {#if user.email_ids.length > 0}
                  {#each user.email_ids as email, index}
                    <p>
                      {index + 1} :
                      <span
                        class="text-brand-foreground dark:text-brand font-medium"
                      >
                        {email.emailAddress}
                      </span>
                    </p>
                  {/each}
                {:else}
                  <p class="text-brand-foreground dark:text-brand font-medium">
                    No emails found
                  </p>
                {/if}
              </Popover.Content>
  ...
  {:else}
    <p
      class="text-brand-foreground dark:text-brand font-medium text-lg text-center pt-5"
    >
      No users found
    </p>
  {/if}
