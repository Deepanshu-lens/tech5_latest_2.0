...
>
  ...
  <Table.Body class=" pb-10">
    {#each allUsers as user}
      <Table.Row
        class="bg-transparent cursor-pointer flex w-max min-w-full items-center gap-4 mt-4 px-3 rounded-lg  border-2 border-solid border-[#e4e4e4]"
      >
        ...
        <Table.Cell class="text-black h-full">
          <span class="flex flex-col font-semibold text-primary">
            <span>Name: {user?.name.length > 0 ? user.name : "-"}</span>
  ...
  <Button class="mr-auto" on:click={handleFeaturesUpdate}>save</Button>
</Table.Root>
<!-- </div>/ -->
